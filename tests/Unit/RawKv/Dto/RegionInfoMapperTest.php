...
class RegionInfoMapperTest extends TestCase
{
    ...
    public function testV2CodecDecodesKeyspaceStartAndEndBoundaries(): void
    {
        $epoch = new RegionEpoch();
        $epoch->setConfVer(1);
        $epoch->setVersion(1);
        $codec = new CodecV2(Mode::Raw, 42, 'tenant-a');
        [, $upperBound] = $codec->encodeRange('', '');

        $region = new Region();
        $region->setId(5);
        $region->setStartKey($codec->encodeRegionKey(''));
        $region->setEndKey($upperBound);
        $region->setRegionEpoch($epoch);

        $info = RegionInfoMapper::fromProto($region, null, $codec);
