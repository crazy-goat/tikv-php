...
### Fixed
- **`Transaction::batchRollback()` and `pessimisticRollbackAll()` now check responses for region and key errors and retry** — previously, rollback RPC responses were silently ignored; region errors (leader change, region split) and key errors (locked, retryable, abort) were swallowed, leaving locks dangling in TiKV until TTL expiry. Both methods now wrap each region's call in the retry executor and surface non-recoverable failures. Locked keys are resolved before retry. (#92)
