...
### Fixed
- **SEC-03 (security)**: `RegionResolver::resolveStoreAddress()` now validates the store address returned by PD before it is used as a gRPC channel target. Any address that is not a bare `host:port` (e.g. `unix:`, `unix-abstract:`, `dns:///` forms) is unconditionally rejected with a distinct `InvalidStoreAddressException` (logged at error level) instead of being passed to the channel constructor. Prevents a rogue or on-path PD from redirecting the client — and the data it writes — to an arbitrary host, port or local Unix socket. (#306)
...
### Added
- `options['allowedStoreHosts']` and `options['storeHostPolicy']` on `RawKvClient::create()`/`TxnKvClient::create()` — opt-in host restrictions for store addresses returned by PD (exact hostnames, DNS suffixes, CIDR ranges, or a custom callable policy). A store address outside the allowed set is rejected with a logged `InvalidStoreAddressException`. Unset by default; only the unconditional `host:port` format check applies then (backward compatible). (#306)
